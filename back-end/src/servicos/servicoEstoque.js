...
function estoqueBaixo(quantidade){return Number(quantidade)<=LIMITE_ESTOQUE_BAIXO;}
...
function registrarSaida(produtoId,quantidade,destinatario,motivo,usuarioId,body={}) {
  ...
  db.exec('BEGIN');try{
    ...
    db.exec('COMMIT');
    const alerta=estoqueBaixo(novo);if(alerta){repositorioEstoque.adicionarAlerta({produto_id:p.id,mensagem:`Estoque baixo para ${p.nome}`});for(const setor of ['COMPRAS','LOGISTICA'])repositorioEstoque.adicionarNotificacao({setor,titulo:'Estoque baixo',mensagem:`${p.nome} atingiu o limite de ${LIMITE_ESTOQUE_BAIXO} itens.`,produto_id:p.id});}
