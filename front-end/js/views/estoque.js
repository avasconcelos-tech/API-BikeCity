...
const produtos = (await getProdutos()).dados || [];
...
['produto-entrada', 'produto-saida', 'produto-devolucao', 'produto-ajuste'].forEach((id) => {
  if ($(id)) preencherProdutos(id);
});
...
function camposRastreio(prefix) {
  ...
  box.innerHTML = '';
  if (!p) return;
  const tipo = (p.tipo_rastreabilidade || 'NENHUMA').toUpperCase();
  if (tipo === 'BATERIA')
    box.innerHTML =
      '<div class="form-row"><div class="form-group"><label>Número de série</label><input id="serie-entrada" required></div><div class="form-group"><label>Data de validade</label><input id="validade-entrada" type="date" required></div></div>';
  else if (tipo === 'MOTOR_CONTROLADOR')
    box.innerHTML =
      '<div class="form-row"><div class="form-group"><label>Número de série</label><input id="serie-entrada" required></div><div class="form-group"><label>Lote</label><input id="lote-entrada" required></div></div>';
  else if (tipo === 'VEICULO')
    box.innerHTML =
      '<div class="form-group"><label>ID único do veículo</label><input id="id-unico-entrada" required></div>';
  else if (tipo === 'PECA_SEGURANCA')
    box.innerHTML =
      '<div class="form-group"><label>Lote</label><input id="lote-entrada" required></div>';
  associarLabels(box);
}
...
$('form-entrada')?.addEventListener('submit', async (e) => {
  ...
  if ($('serie-entrada')) dados.numero_serie = $('serie-entrada').value;
  if ($('validade-entrada')) dados.data_validade = $('validade-entrada').value;
  if ($('lote-entrada')) dados.lote = $('lote-entrada').value;
  if ($('id-unico-entrada')) dados.identificador_unico = $('id-unico-entrada').value;
  ...
  try {
    ...
    $('tbody-estoque').innerHTML =
      lista
        ...
        .join('') || '<tr><td colspan="8">Nenhuma movimentação registrada.</td></tr>';
  } catch (e) {
    $('tbody-estoque').innerHTML =
