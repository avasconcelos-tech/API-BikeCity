...
$('btn-novo-produto').onclick = async () => {
  await carregarFornecedores();
  abrirModal(modal, 'nome');
};
$('btn-fechar-modal').onclick = fecharProduto;
async function carregarFornecedores() {
  const r = await getFornecedores();
  $('fornecedor_id').innerHTML =
    '<option value="">Selecione...</option>' +
    (r.dados || []).map((f) => `<option value="${f.id}">${f.nome}</option>`).join('');
}
...
function abrirEdicao(produto) {
  ...
  $('editar-nome').value = produto.nome || '';
  $('editar-categoria').value = produto.categoria || '';
  $('editar-localizacao').value = produto.localizacao_deposito || '';
  $('editar-custo').value = produto.custo ?? 0;
  ...
  try {
    ...
    $('tbody-produtos').innerHTML =
      lista
        .map(
          (p) =>
            `<tr><td>${p.id}</td><td>${p.codigo_interno || '-'}</td><td>${p.nome}</td><td>${p.categoria}</td><td>${p.localizacao_deposito || '-'}</td><td>R$ ${Number(
              p.custo || 0,
            )
              .toFixed(2)
              .replace(
    ...
    $('tbody-produtos')
      ...
      .forEach((botao) =>
        botao.addEventListener('click', () =>
          abrirEdicao(lista.find((p) => p.id === Number(botao.dataset.produtoId))),
        ),
      );
  } catch (e) {
    console.error(e);
...
$('form-produto').onsubmit = async (e) => {
  ...
  const dados = {
    ...
    localizacao_deposito: $('localizacao_deposito').value,
  ...
  try {
    ...
    mostrarToast(err.message || 'Erro ao cadastrar produto.', 'erro');
  } finally {
    botao.disabled = false;
  }
};
if (obterUsuarioLogado()?.perfil !== 'GERENTE' && obterUsuarioLogado()?.perfil !== 'ANALISTA')
  $('btn-novo-produto').style.display = 'none';
carregarProdutos();
if (!podeEditar) {
  modalEdicao.remove();
} else {
  const fecharEdicao = () => fecharModal(modalEdicao, 'btn-novo-produto');
  associarLabels(modalEdicao);
  configurarModal(modalEdicao, fecharEdicao);
  $('btn-fechar-edicao').onclick = fecharEdicao;
  $('form-editar-produto').onsubmit = async (e) => {
    e.preventDefault();
    const botao = e.target.querySelector('button[type="submit"]');
    botao.disabled = true;
    try {
      await putProduto(produtoEmEdicao.id, {
        nome: $('editar-nome').value,
        categoria: $('editar-categoria').value,
        localizacao_deposito: $('editar-localizacao').value,
        custo: Number($('editar-custo').value),
      });
      mostrarToast('Produto atualizado com sucesso!');
      fecharEdicao();
      await carregarProdutos();
    } catch (err) {
      mostrarToast(err.message || 'Erro ao editar produto.', 'erro');
    } finally {
      botao.disabled = false;
    }
  };
